...
Future<void> patchViews(Directory dir, DatabaseSchemaDiff diff) async {
  ...
  if (toDropGraph.isNotEmpty) {
    ...
    while (toDropGraph.isNotEmpty) {
      ...
      if (dropViews.isNotEmpty) {
        dropViews += '\n\n';
      }
      dropViews += 'DROP VIEW ${node.view.name};';
    ...
    await writeFile(dir, 'drop_views', dropViews);
  }
  ...
  if (toAddGraph.isNotEmpty) {
    ...
    await writeFile(dir, 'create_views', createViews);
  }
